...
## 4.4.0 &mdash; 2021-11-09
* [Fixed] Minor fix to `reset:false` behaviour:  When `.ready()` was called a 2nd time, the plugin would apply the config anyway, bypassing `reset:false`.
...
* [Changed][Android] Update okhttp default version from `3.12.+` -> `3.14.+`.  Bump `play-services-location:16.+` -> `18.+`.

## 4.1.1 &mdash; 2021-06-10
...
* [Fixed][Android] Fix build.gradle failing to detect Capacitor 3 apps (capacitor.config.ts)
